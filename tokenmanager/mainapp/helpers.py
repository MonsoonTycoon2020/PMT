...
def retrieve_sender():
    return ('IDQR4TFQ6BSJO76UH4MPVRHO7M3IWFD3WY3NXSRHVBTQ5P4AMZOS2FQ7BE')
...
def schedule_transaction(sender, receiver, passphrase, amount, asset_id, note):
    """Create and sign asset transfer for future round

    Returned non-empty tuple carries field where error was raised and description, as well as results
    If the first item is None then the error is non-field/integration error.
    Returned three-tuple of empty strings marks successful transaction.
    """

    client = _algod_client()
    params = client.suggested_params()

    unsigned_txn = AssetTransferTxn(sender, params, receiver, amount, asset_id,  None, None, note.encode(), )
